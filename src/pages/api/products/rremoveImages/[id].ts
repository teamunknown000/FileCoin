...
export default async function handler(
    ...
    if(req.method === HttpMethods.put) {
        ...
        if(!body.images.length) {
            return res.status(400).send(Errors.generateResponse(Errors.LENGTH_ZERO, false))
        }
        const product = await Product.findById(id)
        if(!product) {
            return res.status(400).send(Errors.generateResponse(Errors.OBJECT_NOT_FOUND, false))
        }
        product.set("images", ((product.images || []) as string[]).filter(e => !body.images.includes(e)))
